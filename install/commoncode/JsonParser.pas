...
begin
    Result := 0;
    S := '';
    if JsonParser.Ch = '-' then begin
        S := '-';
        Next(JsonParser, '-');
    end;
    while (JsonParser.Ch >= '0') and (JsonParser.Ch <= '9') do begin
        S := S + JsonParser.Ch;
        Next(JsonParser, #0);
    end;
    if JsonParser.Ch = '.' then begin
    ...
    if (JsonParser.Ch = 'e') or (JsonParser.Ch = 'E') then begin
        S := S + JsonParser.Ch;
        Next(JSonParser, #0);
        if (JsonParser.Ch = '-') or (JsonParser.Ch = '+') then begin
            S := S + JsonParser.Ch;
            Next(JsonParser, #0);
        end;
        while (JsonParser.Ch >= '0') and (JsonParser.Ch <= '9') do begin
...
begin
    ...
    if JsonParser.Ch = '"' then begin
        while Next(JsonParser, #0) <> #0 do begin
            if JsonParser.Ch = '"' then begin
                Next(JsonParser, #0);
                Exit;
            end;
...
begin
    ...
    if JsonParser.Ch = '[' then begin
        ...
        while JsonParser.Ch <> #0 do begin
            Inc(N);
            SetLength(Result, N);
...
begin
    ...
    Lines.Add(IS0 + '[');
    for I := 0 to Length(Output.Arrays[Index]) - 1 do begin
...
begin
    ...
    IS1 := IndentString(Indent + 1);
    Lines.Add(IS0 + '{');
    for I := 0 to Length(Output.Objects[Index]) - 1 do begin
