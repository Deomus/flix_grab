...
WelcomeLabel2=次へ」をクリックすると、エンドユーザーライセンス契約とプライバシーポリシーの条件に同意したことになります。
...
; *** FreeGrabAppMessages
welcomeLabel1Custom=%nバージョン %1
readEULA=FreeGrabAppのEULAとプライバシーポリシーを読む
installOldVer=古いバージョンのプログラムをインストールしようとしています。今すぐ最新版をダウンロードしますか？
checkInternetConnection=インストーラーがコンポーネントを確認できません。%nインターネット接続を確認し、「OK」を押すか「キャンセル」をクリックしてセットアップを終了してください。
checkInternetConnectionOk=インストーラがコンポーネントを確認できません。%nインターネット接続を確認してください。
productslbl=無料