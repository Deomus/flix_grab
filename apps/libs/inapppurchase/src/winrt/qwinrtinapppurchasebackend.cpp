...
public:
    explicit QWinRTInAppPurchaseBackendPrivate(QWinRTInAppPurchaseBackend *p)
        : q_ptr(p)
        , roinit(RO_INIT_MULTITHREADED)
    {
        //http://zabkat.com/blog/winrt-win32-store-registration.htm
...
void QWinRTInAppPurchaseBackend::purchaseProduct(QWinRTInAppProduct *product)
{
    ...
    if (product->identifier() == qt_win_app_identifier) {
        // Buy the app itself
        hr = runOnXamlThread([d, product, this]() {
            HRESULT hr;
        ...
        hr = runOnXamlThread([d, product, &productId, this]() {
            HRESULT hr;
