...
namespace SoftActivate.Licensing
{
    ...
    public class LicenseTemplate
    {
        ...
        enum XML_STATE { STATE_ROOT, STATE_LICENSE_KEY, STATE_DATA_FIELDS, STATE_VALIDATION_FIELDS, STATE_PRIVATE_KEY, STATE_PUBLIC_KEY };

        public void LoadXml(string xmlTemplate)
        {
            XML_STATE state = XML_STATE.STATE_ROOT;
            string value = "";

            _ValidationDataSize = 0;

            using (XmlReader xml = XmlReader.Create(new StringReader(xmlTemplate)))
            {
                while (xml.Read())
                {
                    switch (xml.NodeType)
                        ...
                        case XmlNodeType.EndElement:
                            ...
                            if (!xml.IsEmptyElement && !(xml.NodeType == XmlNodeType.EndElement))
                            {
                                switch (xml.Name)
                                {
                                        ...
                                        _NumGroups = int.Parse(xml.GetAttribute("characterGroups"));
                                        _CharsPerGroup = int.Parse(xml.GetAttribute("charactersPerGroup"));
                                        _GroupSeparator = xml.GetAttribute("groupSeparator") ?? "-";
                                        _Header = xml.GetAttribute("header") ?? "";
                                        _Footer = xml.GetAttribute("footer") ?? "";
                                        _KeyEncoding = (xml.GetAttribute("encoding") == "base64") ? LicenseKeyEncoding.Base64 : LicenseKeyEncoding.Base32X;
                                        
                                        state = XML_STATE.STATE_LICENSE_KEY; 
                                        
                                        break;

                                    case "Signature":
                                        _SignatureSize = int.Parse(xml.GetAttribute("size"));
                                        _DataSize = (_NumGroups * _CharsPerGroup) * (int)_KeyEncoding - _SignatureSize;
                                        break;

                                    case "DataFields": state = XML_STATE.STATE_DATA_FIELDS; break;
                                    case "ValidationFields": state = XML_STATE.STATE_VALIDATION_FIELDS; break;
                                    case "SignaturePrivateKey": state = XML_STATE.STATE_PRIVATE_KEY; break;
                                    case "SignaturePublicKey": state = XML_STATE.STATE_PUBLIC_KEY; break;
                            ...
                            else
                            {
                                switch (xml.Name)
                                {
                                    case "Field":
                                        ...
                                        int offset = int.Parse(xml.GetAttribute("offset") ?? "-1");
                                        string typeStr = xml.GetAttribute("type") ?? "raw";
                                        ...
                                        switch (state)
                                        {
                                            case XML_STATE.STATE_DATA_FIELDS:
                                                keyDataMap.AddField(name, (BitStruct.FieldType)type, size, offset);
                                                break;

                                            case XML_STATE.STATE_VALIDATION_FIELDS:
                                                validationDataMap.AddField(name, (BitStruct.FieldType)type, size, offset);
                                    ...
                                    case "SignaturePrivateKey":
                                        SetPrivateKey(value);
                                        break;

                                    case "SignaturePublicKey":
                                        SetPublicKeyCertificate(value);
                                        break;

                                    case "SigningServiceUrl":
                                        SetSigningServiceUrl(value);
                                        break;

                                    case "SigningServiceTemplateId":
                                        SetSigningServiceTemplateId(int.Parse(value));
                                        break;
                                }
                            }
...
/*
           ...
           if ((e = (XmlElement)x.SelectSingleNode("/" + rootElementName + "/LicenseKey/Signature/SignaturePublicKey")) != null)
           {
               if (templateVersion == 1)
               {
                   ...
                   Key key = new Key(e.InnerText, signatureKeyType);
                   SetPublicKeyCertificate(Certificate.Generate(SignatureSize, DateTime.UtcNow.AddYears(10), null, Convert.ToBase64String(key.Export()), null, null, certificateAuthorityUrl).ToString());
               }
               else
                   if (templateVersion <= 3)
                   {
        ...
        public string SaveXml(bool savePrivateKey)
        {
            ...
            x.WriteStartDocument();
                ...
                x.WriteAttributeString("version", Version.ToString());
                    ...
                    x.WriteAttributeString("encoding", "BASE32X");
