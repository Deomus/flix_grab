        ...
        internal static Certificate Generate(int signatureSize, DateTime? expirationDate, Key privateKey, Key publicKey, Key signingPrivateKey, Certificate signingCertificate, string certificateAuthorityUrl)
        {
            ...
            if (signingKey == null)
            {
                if (!String.IsNullOrEmpty(certificateAuthorityUrl))
                {
                    ...
                    Certificate issuedCert = new Certificate(client.DownloadString(certificateAuthorityUrl + "?csr=" + Uri.EscapeDataString(Convert.ToBase64String(certBits.Buffer)) + (String.IsNullOrEmpty(SDKRegistration.LicenseKey) ? "" : "&sdklicensekey=" + SDKRegistration.LicenseKey)));
