...
#include "scoped_handle_verifier.h"
...
void* GetHandleVerifier() {
    std::cout << "GetHandleVerifier..." << std::endl;
...
void ThreadSafeAssignOrCreateScopedHandleVerifier(
    ...
    bool enabled) {
  //todo: AutoNativeLock lock(*GetLock());
...
void ScopedHandleVerifier::InstallVerifier() {
    if (g_active_verifier)
        return;
    g_active_verifier = new ScopedHandleVerifier(true);
...
bool ScopedHandleVerifier::CloseHandle(HANDLE handle) {
  ...
  closing_.Set(false);*/
  //std::cout << "ScopedHandleVerifier::CloseHandle..." << std::endl;
  return true;
}
...
void ScopedHandleVerifier::StartTracking(HANDLE handle,
                                         ...
                                         const void* pc1,
                                         const void* pc2) {
    //std::cout << "ScopedHandleVerifier::StartTracking..." << std::endl;
...
void ScopedHandleVerifier::StopTracking(HANDLE handle,
                                        ...
                                        const void* pc1,
                                        const void* pc2) {
    //std::cout << "ScopedHandleVerifier::StopTracking..." << std::endl;
  /*if (!enabled_)
    return;
...
void ScopedHandleVerifier::OnHandleBeingClosed(HANDLE handle) {
    //std::cout << "ScopedHandleVerifier::OnHandleBeingClosed..." << std::endl;
    if (!enabled_)
    return;
...
HMODULE ScopedHandleVerifier::GetModule() const {
    std::cout << "ScopedHandleVerifier::GetModule..." << std::endl;
