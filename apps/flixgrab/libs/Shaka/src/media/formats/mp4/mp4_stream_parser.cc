...
int MP4StreamParser::GetSampleWidth(int track_id) const {
    if (moov_ && track_id < moov_->tracks.size()) {
        const auto& v = moov_->tracks[track_id].media.information.sample_table.description.video_entries;
        if (v.size() > 0)
            return v.begin()->width;
    }
    return INT_MAX; // no samples found
}

int MP4StreamParser::GetSampleHeight(int track_id) const {
    if (moov_ && track_id < moov_->tracks.size()) {
        const auto& v = moov_->tracks[track_id].media.information.sample_table.description.video_entries;
        if (v.size() > 0)
            return v.begin()->height;
    }
    return INT_MAX; // no samples found
}
