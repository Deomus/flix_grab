...
namespace shaka {
    namespace media {
        ...
        HttpFileShaka::HttpFileShaka(const char* file_name, const char* mode)
            : File(file_name)
            , file_mode_(mode)
            , size_(0) {
            http_ = std::make_unique<utils::Http>();
        }
        HttpFileShaka::~HttpFileShaka() {
            Close();
        }

        bool HttpFileShaka::Open() {
...
#define CHUNK_SIZE              (16*1024)
        int64_t HttpFileShaka::Read(void* buffer, uint64_t length) {
            DCHECK(buffer != NULL);
        ...
        int64_t HttpFileShaka::Write(const void* buffer, uint64_t length) {
           
            return 0;
        }

        int64_t HttpFileShaka::Size() {
            
            return size_;
        }

        bool HttpFileShaka::Flush() {
           
            return true;
        }

        bool HttpFileShaka::Seek(uint64_t position) {
        ...
        bool HttpFileShaka::Tell(uint64_t* position) {
            *position = position_;
            return true;
        }
