...
void InitializeNowFunctionPointer() {
  ...
  //not implemented: CPU cpu;
  const TimeTicksNowFunction now_function =
      (ticks_per_sec.QuadPart <= 0 || true /*!cpu.has_non_stop_time_stamp_counter()*/)
...
bool ThreadTicks::IsSupportedWin() {
    static bool is_supported = false; //Not implemented: CPU().has_non_stop_time_stamp_counter();
  return is_supported;
}
