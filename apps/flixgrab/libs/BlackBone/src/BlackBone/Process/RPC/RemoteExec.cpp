...
DWORD RemoteExec::ExecDirect(ptr_t pCode, ptr_t arg, HANDLE hToken /*= null*/)
{
    ...
    file_logger << "thread id: " << thread.id() << ", valid: " << thread.valid() << std::endl;
#endif
    auto b = thread.Join();
#ifdef FILELOGGER
    file_logger << "join: " << b << std::endl;
#endif
    auto st = thread.ExitCode();
#ifdef FILELOGGER
    file_logger << "thread st: " << st << std::endl;
#endif
    return st;
}
