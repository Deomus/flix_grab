...
Thread ProcessThreads::CreateNew(ptr_t threadProc, ptr_t arg, enum CreateThreadFlags flags /*= NoThreadFlags*/, HANDLE hToken /*= nullptr*/)
{
#ifdef FILELOGGER
    ...
    HANDLE hThd = NULL;
    NTSTATUS st;
    if (!NT_SUCCESS(st = _core.native()->CreateRemoteThreadT(hThd, threadProc, arg, flags, THREAD_ALL_ACCESS)))
    {
#ifdef FILELOGGER
        file_logger << "CreateRemoteThreadT THREAD_ALL_ACCESS st: " << st << std::endl;
#endif
        // Ensure full thread access
        if (NT_SUCCESS(st = _core.native()->CreateRemoteThreadT(hThd, threadProc, arg, flags, THREAD_QUERY_LIMITED_INFORMATION)))
        {
#ifdef FILELOGGER
            file_logger << "CreateRemoteThreadT THREAD_QUERY_LIMITED_INFORMATION st: " << st << std::endl;
#endif
            if (Driver().loaded()) {
                st = Driver().PromoteHandle(GetCurrentProcessId(), hThd, THREAD_ALL_ACCESS);
#ifdef FILELOGGER
                file_logger << "CreateRemoteThreadT Driver().PromoteHandle st: " << st << std::endl;
#endif
            }
        }
    }
