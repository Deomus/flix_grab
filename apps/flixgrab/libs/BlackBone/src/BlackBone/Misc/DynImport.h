...
public:
    ...
    template<typename T, typename... Args>
    inline static auto safeCall( const std::string& name, Args&&... args ) -> typename std::invoke_result<T, Args...>::type
    {
        auto pfn = DynImport::get<T>( name );
        return pfn ? pfn( std::forward<Args>( args )... ) : (std::invoke_result<T, Args... >::type)(0);
    }
