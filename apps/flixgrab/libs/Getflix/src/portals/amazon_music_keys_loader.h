...
class AmazonMusicKeysLoader : public QObject, 
	public SessionMessageCallback
{
	Q_OBJECT
public:
	AmazonMusicKeysLoader(QObject* p = nullptr): QObject(p) {};
	~AmazonMusicKeysLoader();
public:
...
//	void                error(const Status&);
	void                keysLoaded();
	QString				getPostData(QByteArray& challenge);
...
protected:
	virtual void onLoadResponseFromPRS(const WebContent* content);
