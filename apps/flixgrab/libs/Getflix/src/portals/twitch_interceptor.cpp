...
TwitchInterceptor::TwitchInterceptor(QWebEnginePage* page, QNetworkAccessManager* nam, QObject* p)
...
TwitchInterceptor::~TwitchInterceptor() {
    qDebug() << "Interceptor: ~TwitchInterceptor";
}

void TwitchInterceptor::interceptRequest(QWebEngineUrlRequestInfo& info) {
    ...
    if (info.resourceType() == QWebEngineUrlRequestInfo::ResourceTypeScript) {
        if (qstr.contains("amazon-ivs-wasmworker.min", Qt::CaseInsensitive)) {
...
void TwitchInterceptor::initRequest(WebContent* request) {
...
void TwitchInterceptor::startPage(bool forced_reload_page) {
...
void TwitchInterceptor::finishPage(bool ok) {
    Q_UNUSED(ok);
}

void TwitchInterceptor::initialize() {
    qDebug() << "TwitchInterceptor::initialize ...";
...
void TwitchInterceptor::requestGuestToken(const QUrl& url) {
    Q_UNUSED(url);
...
void TwitchInterceptor::requestMetadata(const QUrl& url) {
    if (guest_token_.isEmpty() 
        || metadata_ != nullptr)
        return;
...
void TwitchInterceptor::loadVmap(const QString& url, VideoInfo& video_info, TracksInfo& tracks_info) {
    QString data;
                    ...
                    if (xml.name() == "videoVariant") {  //video quality in block - <videoVariant>
                        ...
                        if (attributes.hasAttribute("url")) {
                            ...
                            if (QRegExp(video_re_str).exactMatch(url)) {
                                ...
                                normalizeTrack(video_track);
                                tracks_info.calculateTrackSize(video_track, video_info_);
                                normalizeVideoTrack(video_track);
...
void TwitchInterceptor::parseM3U8(const QString & data, const QString & url, TracksInfo & tracks_info) {
...
void TwitchInterceptor::requestMain(const QUrl& url) {
...
//    }, Qt::QueuedConnection);
    ...
    Q_UNUSED(url);
    main_script_->fetch(QUrl("qrc:///scripts/twitch/main_script.js"));
    scripts_.addAsync(main_script_);
    scripts_.run(page_);
}

Q_INVOKABLE void TwitchInterceptor::interceptConfig(const QString& jsonStr, const QString& confName) {
